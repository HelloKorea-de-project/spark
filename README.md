...
최종적으로 72개 이상으로 셔플 파티션을 설정하였을 경우, 가장 빠른 연산 속도를 보인다는 것을 확인할 수 있었다.

#### 4) Salting을 통한 Data skew 해소 ####

현실의 데이터는 특정 컬럼을 기준으로 어느 한 값에 데이터가 몰려 있는 data skew 현상을 쉽게 관찰할 수 있는데, 이러한 컬럼을 통해 조인과 같은 셔플링 연산을 수행할 때, 특정 파티션에 데이터가 쏠려 memory를 넘어서는 spill이 일어나게 되고 이는 연산 속도 저하의 큰 원인 중 하나가 된다.

Salting은 이러한 편향된 데이터에 소금을 뿌려 균형을 맞춘다는 개념으로 볼 수 있다.
특정 파티션에만 데이터가 몰리는 것을 방지하기 위해 셔플링 연산에 참여하는 key를 더 추가하는데, 이를 salt key라 한다.
여기서는 난수를 생성하는 컬럼을 추가하여 salt key로 활용하였다.

결론적으로 6개의 파티션에서 발생하던 메모리 spill을 해결하여 연산 수행 시간을 약 30% 개선시킬 수 있었다.
