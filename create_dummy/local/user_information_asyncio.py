...
async def upload_to_s3(idx, all_user_data):
...
async def create_user(idx, num_of_users, start_time):
    ...
    await upload_to_s3(idx, user_data)
    
    
async def generate_task(loop, start_time):
    tasks = [create_user(idx, 100000, start_time) for idx in range(loop*10, (loop+1)*10)]
    await asyncio.gather(*tasks)

def main():
    start_time = datetime.now()
    for loop in range(10):
        asyncio.run(generate_task(loop, start_time))
...
result = pd.read_csv(StringIO(obj['Body'].read().decode('utf-8')))
