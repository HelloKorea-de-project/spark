...
airline_ticket_df = (spark.read.csv(csv_path, header=True, inferSchema=True)
                     .select('2024-08-05 09:55:00', 'KIX'))
...
def pick_airline_ticket_row():
    ...
    return depart_time.strftime('%Y-%m-%d'), depart_airport_code, 'ICN'
...
# DataFrame을 S3에 저장
df.write.parquet("s3://hellokorea-extra-data-zone/source/web_log/airline_search_log.parquet")
